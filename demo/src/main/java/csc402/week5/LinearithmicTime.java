...
public class LinearithmicTime {
    public static void main(String[] args) {
        int[] array = {5, 3, 1, 4, 2};
        IntegerMergeSort(array, 0, array.length-1);
    ...
    public static void IntegerMergeSort(int[] array, int left, int right) {
        if (left < right) {
            int mid = (left + right) / 2;

            // Recursively sort the left and right halves
            IntegerMergeSort(array, left, mid);
            IntegerMergeSort(array, mid + 1, right);

            // Merge the sorted halves
            IntegerMerge(array, left, mid, right);
        }
    }

    public static void IntegerMerge(int[] array, int left, int mid, int right) {
